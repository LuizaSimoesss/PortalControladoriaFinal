...
export default function DreForecastContabilPage() {
  ...
  const [filtros, setFiltros] = usePersistedData<Filtros>("portal_dre_filtros_forecast_ctb", filtrosVazios);
  ...
  useEffect(() => {
    setFcMap(buildForecastMap("contabil", ano, crFiltroSet, filtros.cenarioNome || undefined));
  ...
  function exportar() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, `DRE Contábil Forecast ${ano}`);
    XLSX.writeFile(wb, `DRE_Forecast_Contabil_${ano}.xlsx`);
  }
  ...
  return (
    <div>
      <PageHeader title="Demonstração de Resultado Contábil" subtitle={`Forecast · ${anoAtivo}`} />

...
import type { LancamentoFinanceiro, Fechamento } from "@/lib/mockData";
...
export default function DreComparativoPage() {
  ...
  const lancamentosBaseOrc = useMemo(() => {
    if (!dataLoaded) return [];
    return lancamentos.filter(l => l.tipo === "orcado");
  }, [lancamentos, dataLoaded]);

  const lancamentosPorPeriodoOrc = useMemo(() => {
    const map = new Map<string, LancamentoFinanceiro[]>();
    for (const l of lancamentosBaseOrc) {
      const bucket = map.get(l.periodo);
      if (bucket) bucket.push(l);
      else map.set(l.periodo, [l]);
    }
  ...
  const valoresPorMesOrc = useMemo(() => {
    ...
    return MESES.map((_, mi) => {
      const p = `${ano}-${String(mi + 1).padStart(2, "0")}`;
      if (p < periodoInicio || p > periodoFim) return EMPTY_PERIOD;
      return computePeriod(dre, lancamentosPorPeriodoOrc.get(p) ?? []);
    });
  }, [dre, lancamentosPorPeriodoOrc, filtros]);
  ...
  const totalLancamentos =
    ...
    lancamentosBaseOrc.filter(l => l.periodo >= filtros.periodoInicio && l.periodo <= filtros.periodoFim).length;
                  ...
                  return (
                    <tr key={item.id}
                        ...
                        return (
                          <React.Fragment key={ci}>
                            ...
                            <td className={`px-3 py-2.5 text-right tabular-nums whitespace-nowrap${ci > 0 ? " border-l border-gray-100" : ""}`}>
                              {orc !== 0
                                ? <span className={orc < 0 ? (s.dark ? "text-red-300" : "text-red-600") : ""}>{fmtInt(orc)}</span>
                                : <span style={{ opacity: 0.18 }}>—</span>}
                            </td>
                            {/* Realizado */}
                            <td className="px-3 py-2.5 text-right tabular-nums whitespace-nowrap">
                              {real !== 0
                                ? <span className={real < 0 ? (s.dark ? "text-red-300" : "text-red-600") : ""}>{fmtInt(real)}</span>
                                : <span style={{ opacity: 0.18 }}>—</span>}
                            </td>
                            {/* Variação % */}
                            <td className="px-3 py-2.5 text-right tabular-nums whitespace-nowrap">
                      ...
                      <td className="px-3 py-2.5 text-right tabular-nums whitespace-nowrap border-l border-gray-100"
                        ...
                        {totalOrc !== 0
                          ? <span className={totalOrc < 0 ? (s.dark ? "text-red-300" : "text-red-600") : ""}>{fmtInt(totalOrc)}</span>
                          : <span style={{ opacity: 0.18 }}>—</span>}
                      </td>
                      <td className="px-3 py-2.5 text-right tabular-nums whitespace-nowrap"
                        style={{ fontWeight: isSubtotal ? "700" : "500" }}>
                        {totalReal !== 0
                          ? <span className={totalReal < 0 ? (s.dark ? "text-red-300" : "text-red-600") : ""}>{fmtInt(totalReal)}</span>
                          : <span style={{ opacity: 0.18 }}>—</span>}
                      </td>
                      <td className="px-3 py-2.5 text-right tabular-nums whitespace-nowrap">

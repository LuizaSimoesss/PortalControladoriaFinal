...
import type { LancamentoFinanceiro, NaturezaRow, CentroResultadoRow } from "@/lib/mockData";
...
export default function DreComparativoAnosPage() {
  const [dreRaw] = usePersistedData<DemoItem[]>("portal_dre", []);
  ...
  const [filtros,    setFiltros]    = usePersistedData<Filtros>("portal_dre_filtros_comparativo_anos", filtrosVazios);
  ...
  // Corrige anoA/anoB quando os anos disponíveis carregam
  useEffect(() => {
  ...
  if (!dataLoaded) {
    return (
      <div>
        <PageHeader title="Demonstração de Resultado do Exercício" subtitle="Comparativo Ano × Ano" />
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="text-sm" style={{ minWidth: "max-content", width: "100%", borderCollapse: "separate", borderSpacing: 0 }}>
              <thead>
                {/* Agrupamento — quando não mensal e não somente total */}

...
function ComposicaoModal({ linha, ano, onSave, onAutoSave, onClose }: {
  ...
  const [tipoAdicao, setTipoAdicao] = useState<TipoAdicao>("centro");
    ...
    <Modal title={`Composição · ${linha.descricao}`} onClose={handleClose} xlWide>
      <div className="space-y-3">
        <div className="grid grid-cols-3 gap-1 p-1 bg-gray-100 rounded-lg">
          {(["centro", "centro_parceiro", "centro_projeto"] as TipoAdicao[]).map(t => (
...
const STORAGE_KEY = "portal_orcamento_gastos_pacote_incentivos_comerciais";
const TITULO      = "Pacote de Incentivos Comerciais";

export default function OrcamentoGastosPacoteIncentivosComerciaisPage() {
  ...
  function setCentroLinha(bid: string, sid: string, lid: string, centroResultadoId: string) {
    setBlocos(b => b.map(bl => bl.id !== bid ? bl : { ...bl, subBlocos: bl.subBlocos.map(s => s.id !== sid ? s : { ...s, linhas: s.linhas.map(l => l.id !== lid ? l : { ...l, centroResultadoId }) }) }));
  }
  function exportar() {

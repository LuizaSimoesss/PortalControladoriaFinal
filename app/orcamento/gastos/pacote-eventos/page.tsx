...
const STORAGE_KEY = "portal_orcamento_gastos_pacote_eventos";
const TITULO      = "Pacote de Eventos";

export default function OrcamentoGastosPacoteEventosPage() {
                          ...
                          <table className="text-sm" style={{ width: tableWidth, tableLayout: "fixed", borderCollapse: "separate", borderSpacing: 0 }}>
                            ...
                            <tbody>
                              ...
                              {sub.linhas.map(linha => {
                                ...
                                // Extrai IDs dos operandos da multiplicação (legacy formula ou novo formulaExpr)
                                const calcMulIds = (() => {
                                  if (linha.tipo !== "calculado") return null;
                                  if (linha.formula?.op === "*" && linha.formula.left.valorFixo === undefined && linha.formula.right.valorFixo === undefined)
                                    return { lId: linha.formula.left.linhaId, rId: linha.formula.right.linhaId };
                                  if (linha.formulaExpr?.length === 3) {
                                    const [t0, t1, t2] = linha.formulaExpr;
                                    if (t0.t === "ref" && t1.t === "op" && t1.v === "*" && t2.t === "ref")
                                      return { lId: t0.id, rId: t2.id };
                                  }
                                  return null;
                                })();
                                const hasDetail = (linha.tipo === "digitado" && (linha.composicao?.length ?? 0) > 0) ||
                                  (calcMulIds !== null &&
                                    (allLinhasMap.get(calcMulIds.lId)?.composicao?.length ?? sub.linhas.find(x => x.id === calcMulIds.lId)?.composicao?.length ?? 0) > 0 &&
                                    (allLinhasMap.get(calcMulIds.rId)?.composicao?.length ?? sub.linhas.find(x => x.id === calcMulIds.rId)?.composicao?.length ?? 0) > 0);
                                ...
                                return (<React.Fragment key={linha.id}>
                                  ...
                                  {linhaExpanded && calcMulIds !== null && (() => {
                                    const lL = allLinhasMap.get(calcMulIds.lId) ?? sub.linhas.find(x => x.id === calcMulIds.lId);
                                    const rL = allLinhasMap.get(calcMulIds.rId) ?? sub.linhas.find(x => x.id === calcMulIds.rId);

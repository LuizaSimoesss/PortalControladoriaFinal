...
function ComposicaoModal({ linha, ano, onSave, onAutoSave, onClose }: {
  ...
  const semCR = items.filter(i => !i.centroId);
  const [massaCR, setMassaCR] = useState("");
  const [massaOpen, setMassaOpen] = useState(false);
  function aplicarCREmMassa() {
    if (!massaCR) return;
    setItems(p => p.map(i => i.centroId ? i : { ...i, centroId: massaCR }));
    setMassaOpen(false);
    setMassaCR("");
  }
        ...
        {semCR.length > 0 && (
          <div className="flex items-center gap-3 px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg">
            <span className="text-xs text-amber-700 font-medium flex-1">
              ⚠️ {semCR.length} item{semCR.length !== 1 ? "s" : ""} sem Centro de Resultado → não aparece{semCR.length !== 1 ? "m" : ""} nas colunas Por BU da DRE
            </span>
            <button
              type="button"
              onClick={() => setMassaOpen(v => !v)}
              className="flex-shrink-0 text-xs font-semibold text-amber-700 border border-amber-300 rounded px-2 py-1 hover:bg-amber-100 transition-colors">
              {massaOpen ? "Cancelar" : "Definir CR em massa"}
            </button>
          </div>
        )}
        {massaOpen && (
          <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg">
            <span className="text-xs font-semibold text-blue-700 flex-shrink-0 whitespace-nowrap">Aplicar CR a todos sem CR:</span>
            <div className="flex-1">
              <SearchableSelect value={massaCR} onChange={setMassaCR}
            ...
            <button type="button" onClick={aplicarCREmMassa} disabled={!massaCR}
              className="flex-shrink-0 px-3 py-1.5 text-xs font-semibold text-white rounded-lg disabled:opacity-40 transition-colors"
                              ...
                              item.centroId !== undefined ? (
                                ...
                                <div className="flex gap-1 items-center flex-wrap">
                                  <input autoFocus value={editingDesc} onChange={e => setEditingDesc(e.target.value)}
                                    ...
                                    className="flex-1 min-w-[120px] text-sm border border-amber-300 rounded px-1.5 py-0.5 focus:outline-none focus:ring-1 focus:ring-amber-400 bg-amber-50" />
                                  <div className="flex-1 min-w-[110px]"><SearchableSelect value={editingCentro} onChange={setEditingCentro} options={centroOptions} placeholder="CR (opcional)" emptyLabel="— sem CR —" className="border border-amber-300 rounded px-2 py-0.5 text-xs bg-amber-50" /></div>
                                  <button onClick={commitEdit} className="flex-shrink-0 px-2 py-1 text-xs font-medium text-white rounded bg-amber-500 hover:bg-amber-600">OK</button>
                                  <button onClick={() => setEditingId(null)} className="flex-shrink-0 px-2 py-1 text-xs text-gray-600 bg-gray-200 rounded hover:bg-gray-300">✕</button>
                                </div>
                              )
...
const STORAGE_KEY = "portal_orcamento_gastos_pacote_pessoal";
const TITULO      = "Pacote de Pessoal";

export default function OrcamentoGastosPacotePessoalPage() {

      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {MESES.map((m, mi) => {
            ...
            return (
            <div key={mi}>
              <label className="block text-xs font-medium mb-1" style={{ color: editable ? undefined : "#9ca3af" }}>{m}{!editable && <span className="ml-1 text-[10px] font-normal text-gray-400">(real.)</span>}</label>
  ...
  function commitEdit() {
    if (editingId) {
      setItems(p => p.map(i => {
        ...
        return { ...i, descricao: editingDesc.trim() || i.descricao };
      }));
    }
    setEditingId(null);
  }
...
const ORCAMENTO_KEY = "portal_orcamento_gestao_recursos";
const CENARIOS_KEY  = "portal_forecast_receita_gestao_recursos_cenarios";
const VALORES_KEY   = "portal_forecast_receita_gestao_recursos_valores";
const TITULO        = "Gestão de Recursos";
...
export default function ForecastReceitaGestaoRecursosPage() {
  ...
  const dreGerMap = useMemo(() => {
    const m = new Map<string, DemoItem>();
    for (const item of loadData<DemoItem[]>("portal_dre", [])) m.set(item.id, item);
    return m;
  }, []);
  const dreCtbMap = useMemo(() => {
    const m = new Map<string, DemoItem>();
    for (const item of loadData<DemoItem[]>("portal_dre_contabil", [])) m.set(item.id, item);
    return m;
  }, []);
  const blocos = useMemo<Bloco[]>(() => {
    ...
    const orcLinhasMap = new Map<string, LinhaOrcamento>();
    for (const b2 of orcBlocos) for (const s2 of b2.subBlocos) for (const l2 of s2.linhas) orcLinhasMap.set(l2.id, l2);
    return orcBlocos.map(b => ({
      ...
      subBlocos: b.subBlocos.map(s => ({
        ...
        linhas: s.linhas.map(l => {
          ...
          // merge cenário composicao values first (DRE override runs after)
          ...
          // DRE inheritance: override realized months from lancamentos
          // priority: DRE Gerencial → DRE Contábil → naturezaId fallback
          // applies to digitado AND calculado lines
          if (
            (l.categoria === "receita" || l.categoria === "gastos") &&
            (l.tipo === "digitado" || l.tipo === "calculado") &&
            realizados.size > 0
          ) {
            const dreItem = (l.demoItemIdGerencial ? dreGerMap.get(l.demoItemIdGerencial) : undefined)
                         ?? (l.demoItemIdContabil  ? dreCtbMap.get(l.demoItemIdContabil)  : undefined);
            const regrasCobrem = dreItem && (hasEffectiveRule(dreItem.regras?.natureza) || hasEffectiveRule(dreItem.regras?.centroResultado));
            const getDRE = regrasCobrem
              ? (periodo: string) => lancamentosRealizados
                  .filter(lf => lf.periodo === periodo && matchesRegra(lf.codnat, dreItem!.regras?.natureza) && matchesRegra(lf.codcencus, dreItem!.regras?.centroResultado))
                  .reduce((acc, lf) => acc + lf.valor, 0)
              : l.naturezaId
                ? (periodo: string) => {
                    const cn = naturezaMap.get(l.naturezaId!);
                    const cc = l.centroResultadoId ? centroMap.get(l.centroResultadoId) : undefined;
                    if (!cn) return 0;
                    return lancamentosRealizados.filter(lf => lf.periodo === periodo && lf.codnat === cn && (!cc || lf.codcencus === cc)).reduce((acc, lf) => acc + lf.valor, 0);
                  }
                : null;
            if (getDRE) {
              if (l.tipo === "calculado") {
                // pre-compute formula for non-realized months; use DRE for realized months
                const newVals: Record<string, number> = {};
                for (let mi2 = 0; mi2 < 12; mi2++) {
                  const periodo = pk(ano, mi2);
                  newVals[periodo] = realizados.has(mi2) ? getDRE(periodo) : evalLinha(l, s.linhas, ano, mi2, orcLinhasMap);
                }
                return { ...l, tipo: "digitado" as const, valores: newVals, formula: undefined, formulaExpr: undefined, composicao };
              }
              if (!composicao || composicao.length === 0) {
                for (const mi of realizados) { valores[pk(ano, mi)] = getDRE(pk(ano, mi)); }
              } else {
                // scale composicao items proportionally to match DRE total
                composicao = composicao.map((ci, _, arr) => {
                  const newVals = { ...ci.valores };
                  for (const mi of realizados) {
                    const periodo = pk(ano, mi);
                    const dreTotal = getDRE(periodo);
                    const compSum = arr.reduce((acc, c) => acc + (c.valores[periodo] ?? 0), 0);
                    newVals[periodo] = compSum !== 0 ? (ci.valores[periodo] ?? 0) / compSum * dreTotal : dreTotal / arr.length;
                  }
                  return { ...ci, valores: newVals };
                });
              }
            }
          }
  ...
  }, [orcBlocos, valoresCenarios, cenarioAtivo, cenarios, lancamentosRealizados, naturezaMap, centroMap, dreGerMap, dreCtbMap, ano]);
  ...
  return (
    <div>
      <PageHeader title="Forecast · Receita" subtitle={TITULO} />
              ...
              {!blocoCollapsed && (
                <div className="divide-y divide-gray-50">
                  {bloco.subBlocos.length === 0 && (
                    <div className="px-5 py-6 text-center text-sm text-gray-400">Nenhum sub-bloco.</div>
                  )}

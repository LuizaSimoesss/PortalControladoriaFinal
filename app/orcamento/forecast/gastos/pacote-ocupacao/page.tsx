...
const ORCAMENTO_KEY = "portal_orcamento_gastos_pacote_ocupacao";
const CENARIOS_KEY  = "portal_forecast_gastos_pacote_ocupacao_cenarios";
const VALORES_KEY   = "portal_forecast_gastos_pacote_ocupacao_valores";
const TITULO        = "Pacote de Ocupação";
...
export default function ForecastGastosPacoteOcupacaoPage() {
  ...
  function addCenario() {
    ...
    setCenarioAtivo(id);
    setModal(null); setNomeModal(""); setCorModal(CORES_CENARIO[0]); setMesesModal([]);
  }
    ...
    <div>
      ...
      <div className="p-6 space-y-4 min-w-max">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              onClick={() => setCenarioAtivo(null)}
              className="px-3 py-1.5 text-xs font-medium rounded-full border transition-colors"
              style={cenarioAtivo === null ? { background: "#1e3a5f", color: "white", borderColor: "#1e3a5f" } : { background: "white", color: "#6b7280", borderColor: "#d1d5db" }}>
              Orçamento Base
            </button>
            {cenarios.map(cen => (
              <button key={cen.id}
                onClick={() => setCenarioAtivo(cen.id)}
                onDoubleClick={() => { setNomeModal(cen.nome); setCorModal(cen.cor); setMesesModal(cen.mesesRealizados ?? []); setModal({ tipo: "cenario-edit", id: cen.id, nome: cen.nome, cor: cen.cor }); }}
                className="px-3 py-1.5 text-xs font-medium rounded-full border transition-colors"
                style={cenarioAtivo === cen.id ? { background: cen.cor, color: "white", borderColor: cen.cor } : { background: "white", color: cen.cor, borderColor: cen.cor }}>
                {cen.nome}
              </button>
            ))}
            <button
              onClick={() => { setNomeModal(""); setCorModal(CORES_CENARIO[cenarios.length % CORES_CENARIO.length]); setMesesModal([]); setModal({ tipo: "cenario-add" }); }}
              className="px-3 py-1.5 text-xs font-medium rounded-full border border-dashed border-gray-300 text-gray-400 hover:border-gray-400 hover:text-gray-600 transition-colors">
              + Cenário
            </button>
          </div>
        ...
        {orcBlocos.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-gray-100 text-center">
            <p className="text-gray-500 font-medium">Nenhum bloco no orçamento base</p>
            <p className="text-gray-400 text-sm mt-1">Configure os blocos na página de Orçamento antes de criar cenários de forecast.</p>
          </div>
        )}
      ...
      {(modal?.tipo === "cenario-add" || modal?.tipo === "cenario-edit") && (
        <Modal title={modal.tipo === "cenario-add" ? "Novo Cenário" : "Editar Cenário"} onClose={() => setModal(null)}>
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Nome do cenário</label>
              <input autoFocus value={nomeModal} onChange={e => setNomeModal(e.target.value)}
                ...
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Ex: Otimista, Conservador, Base..." />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-2">Cor</label>
              <div className="flex gap-2 flex-wrap">
                {CORES_CENARIO.map(cor => (
                  <button key={cor} type="button" onClick={() => setCorModal(cor)}
                    className="w-7 h-7 rounded-full border-2 transition-all"
                    style={{ background: cor, borderColor: corModal === cor ? "#1e3a5f" : "transparent", outline: corModal === cor ? "2px solid #1e3a5f" : "none", outlineOffset: 2 }} />
                ))}
              </div>
            </div>
            <div>
